...
async function startServer() {
  try {
    ...
    // ==================== SEED SỰ KIỆN MẪU ====================
         console.log('Đang seed 5 sự kiện mẫu (không phụ thuộc tổ chức)...');
    await Event.bulkCreate([
      {
        ...
        status: 'pending',
        channels: ['web'],
        organizationId: null, // ← Không cần tổ chức
        organizationName: "A'zone" // ← Tên vẫn hiện trên card
      },
      {
    ...
    console.log('SEED 5 SỰ KIỆN MẪU THÀNH CÔNG! (tên tổ chức vẫn hiện dù không có organizationId)');    
