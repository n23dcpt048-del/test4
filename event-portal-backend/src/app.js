...
async function startServer() {
  try {
    ...
    // RESET BẢNG EVENTS 1 LẦN ĐỂ TẠO MỚI ĐẦY ĐỦ CỘT
    // Sau khi deploy thành công, sửa lại thành { alter: true } hoặc { force: false }
    await sequelize.sync({ force: true });
    console.log('RESET BẢNG EVENTS THÀNH CÔNG! Bảng mới có đầy đủ cột.');

    // SEED UGC (giữ nguyên như cũ)
    ...
    app.listen(PORT, '0.0.0.0', () => {
      ...
      console.log(`Data mẫu đã sẵn sàng: 5 UGC + 5 Events (3 pending, 2 approved)`);
    });
  } catch (error) {
